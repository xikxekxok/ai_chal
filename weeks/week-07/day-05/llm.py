"""Dockhost Inference (OpenAI-compatible) с retries и UsageTracker."""
...
def try_load_llm_config() -> LlmConfig | None:
    api_key = os.environ.get("DOCKHOST_AI_KEY") or os.environ.get("OPENAI_API_KEY")
    if not api_key:
        return None
    base_url = os.environ.get("OPENAI_BASE_URL", "https://inference.dockhost.io/v1").rstrip("/")
    ...
    *,
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            print(
                f"[retry] попытка {attempt}/{max_retries} не удалась ({reason}), жду {wait:.0f}с…",
                flush=True,
            )
            time.sleep(wait)
