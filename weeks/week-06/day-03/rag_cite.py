...
def _extract_json(text: str) -> dict[str, Any]:
    ...
    if match:
        try:
            ...
            pass
    raise ValueError("Cannot parse RAG JSON")
...
def _parse_response(raw: str, *, latency_ms: int) -> RagResponse:
...
def generate_cite_rag(
    ...
    *,
    ...
    try:
        return _parse_response(result.content, latency_ms=result.latency_ms)
    except (ValueError, json.JSONDecodeError) as exc:
        print(f"[error] не удалось распарсить JSON cite-RAG: {exc}", file=sys.stderr)
        print_section("raw-response", result.content, layout="block")
...
def format_rag_summary(response: RagResponse) -> str:
    ...
    return (
        ...
        f"{len(response.citations)} citations · {response.latency_ms} ms"
    )
