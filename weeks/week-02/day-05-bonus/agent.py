...
DEFAULT_SYSTEM_PROMPT = (
    "Ты аналитик по сбору технического задания для стартапа OpossumEats "
    "(MVP — доставка еды для опоссумов). "
    "Веди живое интервью с заказчиком: уточняй детали, задавай вопросы, "
    "структурируй требования, фиксируй решения по ходу разговора. "
    "Не жди, что клиент вывалит всё списком — помогай раскрыть ТЗ диалогом. "
    "Отвечай на русском, содержательно, можно с лёгким юмором про opossumов. "
    "На просьбу «напомни…» — перечисли ключевые решения из начала "
    "(бюджет, срок, стек, заказчик)."
)
...
def complete_silent(config: AgentConfig, messages: list[dict[str, str]]) -> str:
    """Off-books LLM: без TokenTracker, без логирования промпта."""
    response = requests.post(
    ...
    content = choice["message"].get("content") or ""
    if not content:
        raise RuntimeError("LLM вернул пустой ответ (silent).")
