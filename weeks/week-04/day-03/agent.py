...
from mcp_client import McpToolClient, preview_json

TOOL_LOOP_FUSE = 30
MAX_REPEAT_SAME_TOOL = 3

SYSTEM_PROMPT = """\
Ты помощник с доступом к веб-поиску, чтению страниц и планировщику напоминаний.
Используй web_search для поиска информации в интернете.
Используй read_page, чтобы прочитать конкретную страницу по URL.
Используй schedule_once, когда пользователь просит напомнить через N секунд/минут.
Используй schedule_recurring, когда нужна периодическая задача (cron в UTC, 5 полей).
Используй list_jobs, чтобы показать или проверить текущие напоминания.
Используй cancel_job(job_id), чтобы отменить старую задачу перед новой — job_id бери из list_jobs.
Если пользователь просит «перезаписать» напоминалку:
list_jobs → cancel_job → schedule_once или schedule_recurring.
В prompt задачи пиши инструкцию агенту — что сделать при срабатывании (можно искать в сети).
Иногда приходят системные сообщения с префиксом [scheduler] — это срабатывание расписания;
выполни инструкции из них так же, как обычный запрос пользователя.
...
class WebAgent:
    ...
    @classmethod
    def create(cls, config: LlmConfig, mcp: McpToolClient) -> WebAgent:
        return cls(
            config=config,
            mcp=mcp,
            tracker=UsageTracker(),
